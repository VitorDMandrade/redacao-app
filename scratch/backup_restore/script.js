...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function mostrarEixo(eixo) {
        ...
        if (eixo.repertorios) {
            ...
            if (shuffleNext) {
                repsToRender.sort(() => 0.5 - Math.random());
                shuffleNext = false;
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleAnswer(selectedIndex, ex, btnElement) {
        ...
        if (isCorrect) {
            ...
            feedbackHTML += `<strong>Correto! 🎉</strong><br><br>`;
            feedbackHTML += `<strong>Por que está certo?</strong> ${ex.correctSide || ex.explanation}<br><br>`;
        } else {
            ...
            feedbackHTML += `<strong>Incorreto! ❌</strong><br><br>`;
            feedbackHTML += `<strong>Por que você errou?</strong> ${ex.wrongSide || ex.explanation}<br><br>`;
        }
        
        if (ex.concept && ex.conceptExplanation) {
            ...
            feedbackHTML += `<span style="font-size: 0.9em;">${ex.conceptExplanation}</span>`;
            feedbackHTML += `</div>`;
        }

        drillFeedback.innerHTML = feedbackHTML;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnShuffle) {
        btnShuffle.addEventListener('click', () => {
            if (typeof mostrarEixo === 'function') {
                shuffleNext = true;
                if (typeof currentBanca !== 'undefined' && currentBanca.eixosTematicos[currentEixoIndex]) {
                    mostrarEixo(currentBanca.eixosTematicos[currentEixoIndex]);
                } else if (typeof EIXOS_MESTRES !== 'undefined' && EIXOS_MESTRES.length > 0) {
                    mostrarEixo(EIXOS_MESTRES[0]);
                }
            }
        });
    }
    ...
    if (btnExportPdf) {
        btnExportPdf.addEventListener('click', () => {
            ...
            const favs = JSON.parse(localStorage.getItem('redacao_favorite_repertories') || '[]');
            if (favs.length === 0) {
            ...
            favs.forEach(id => {
                ...
                if (typeof REPERTORIOS_COMPLETOS !== 'undefined') {
                    repInfo = REPERTORIOS_COMPLETOS.find(r => r.obra === id);
                }
                if (!repInfo && typeof REPERTORIOS_MAPEADOS !== 'undefined') {
                    for(let i = 0; i < REPERTORIOS_MAPEADOS.length; i++) {
                        const found = REPERTORIOS_MAPEADOS[i].repertorios.find(r => r.obra === id);
